...
export default function EmailotpComponent(props) {
  ...
  const verifyotp = () => {
    ...
    if (otpverify.length < 6) {
      ...
      formDataemail.append("otp", otpverify);
      API.PostEmailEditOtp(formDataemail).then((res) => {
        if (res.status) {
          show();
          exitModal();
          logoutfunc();
  ...
  const resendotp = () => {
    if (counter <= 0) {
      ...
      setResendmsg(true);     
      let formDataemail = new FormData();
      formDataemail.append("user_id", userid);
      API.PostEmailResendOtp(formDataemail)
        .then((res) => {
        })
        .catch((err) => {
        });
        ...
        <div className="d-flex flex-row w-100  mt-3">
          <center>
            <h5>
              ...
              {SettingTexts.toyour} <b>{SettingTexts.emialid}</b>
            </h5>
          </center>
        </div>
      ) : null}
